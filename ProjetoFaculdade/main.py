...
def buscar_livros():
    id_livro = int(input('Id: '))
    for id in livros:
        id_livro, titulo, resumo, autor, editora, ano_publicacao, edicao = id
        print(id)
        if id_livro == livros:
...
def emprestimo():
    ...
    if id_livro in emprest:
        ...
        with open('livrosEmprestados.txt', 'a') as l_emprest:
            for e in livros:
                l_emprest.write(e)
            for n in range(len(livros)):
                if livros[n] in emprest:
                    print('Livro não disponível para empréstimo.')
#                arq_l.write(livros[n])
                print()
...
def relatorio():
    ...
    [ 1 ] Lista dos usuários com livros emprestados e não devolvidos (Informações pendentes do livro)
    [ 2 ] Lista dos livros emprestados e não devolvidos até o momento
    [ 3 ] Lista dos 10 livros mais emprestados no mês (com a quantidade de retiradas)''')
    esc = int(input('O que deseja ver primeiro? '))
    if esc == 1:
        for pde in emprest:
