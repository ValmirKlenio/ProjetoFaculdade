...
def alterar_pessoa():
    ...
    for p in pessoas:
        if p['cpf'] == cpf:
            print(p)
            alterar = input('Deseja fazer a alteração desse cadastro? (S/N) ').upper()
            if alterar == 'S':
                pessoas.remove(p)
...
def alterar_livros():
    ...
    for l in livros:
        if l['Id do Livro'] == id_livro:
            print(l)
            alterar_livro = input('Deseja fazer a alteração desse cadastro? (S/N) ').upper()
            if alterar_livro == 'S':
                livros.remove(l)
                print('Começando a alteração...')
                ...
                livros.append({'Id do Livro': id_livro, 'titulo': titulo, 'resumo': resumo,
                               'autor': autor, 'editora': editora, 'ano de publicação': ano_publicacao,
                               'edição': edicao})
                print('Alterações feitas com sucesso!')
            return
    ...
    if consul == 1:
        ...
        for l in livros:
            if parte_titulo in l['titulo']:
                print(f'A parte do Título {parte_titulo} se encontra no título {l["titulo"]}')
                return
        ...
        for l in livros:
            if autor in l['autor']:
                print(f'O(s) Autor(es) {autor} está(ão) cadastrado(s) no nosso sistema!')
                return
...
def emprestimo():
    ...
    for l in livros:
        if l['Id do Livro'] == id_livro:
            ...
            emprestimos.append({'Id do livro': id_livro, 'Título': titulo, 'Pessoa': pessoa,
                                'Data de saída': data, 'Hora de saída': hora})
...
def devolucao():
    ...
    for e in emprestimos:
        if e['Id do livro'] == id_livro:
            ...
            data = date.today()
            hora = datetime.now().time()
            devolucoes.append({'Id do livro': id_livro, 'Título': titulo, 'Pessoa': pessoa,
                              'Data de devolução': data, 'Hora de devolução': hora})
